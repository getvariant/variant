...
public class ConfigLoader {
	...
	public static Config load(String resourceName, String defaultResourceName) {
		...
		if (overrideResourceName != null && overrideFileName!= null) {
			throw new CoreException.User(ServerMessageLocal.CONFIG_BOTH_FILE_AND_RESOURCE_GIVEN());
		}
		...
		if (overrideResourceName != null) {
			try {
				...
				if (res == null) {
					throw new CoreException.User(ServerMessageLocal.CONFIG_RESOURCE_NOT_FOUND(), overrideResourceName);				
				}
				else {
		...
		// 3. Override file may have been given as file
		   ...
		   InputStream is = null;
			...
			try {
				...
				if (is == null) {
					throw new CoreException.User(ServerMessageLocal.CONFIG_FILE_NOT_FOUND(), overrideFileName);
				}
         }
         catch (Exception e) {
            throw new CoreException.User(ServerMessageLocal.CONFIG_FILE_NOT_FOUND(), overrideFileName);
