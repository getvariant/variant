...
/**
 * Parse time exceptions related to hooks with schema domain.
 * @author Igor
 *
 */
public class ParserSchemaHooksErrorTest extends BaseTestCore {
	...
	/**
	 * STATE_UNSUPPORTED_PROPERTY
	 * @throws Exception
	 */
	@Test
	public void hookNotAllowedInStates() throws Exception {
		...
		assertEquals(1, response.getMessages().size());
		ParserMessage error = response.getMessages().get(0);
		assertEquals(new ParserMessageImpl(ParserError.STATE_UNSUPPORTED_PROPERTY, "hooks", "state1").getText(), error.getText());
		assertEquals(Severity.WARN, error.getSeverity());
	}
