...
class StandaloneServerTest extends StandaloneServerSpec {
   ...
   "Server" should {
      ...
      "fail to deploy petclinic without the postgres driver in ext/" in  {
         ...
         lines(0).message mustBe ServerErrorLocal.OBJECT_INSTANTIATION_ERROR.asMessage("com.variant.server.api.TraceEventFlusherPostgres", "java.lang.reflect.InvocationTargetException")

         // This error goes in the log twice: the first instance has the call stack.
         lines(1).severity mustBe ERROR
         lines(1).message mustBe ServerErrorLocal.OBJECT_INSTANTIATION_ERROR.asMessage("com.variant.server.api.TraceEventFlusherPostgres", "java.lang.reflect.InvocationTargetException")
