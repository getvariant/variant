    	...
    	"use defaults" in {
         ...
         val eventWriterMaxDelayMillis = schema.eventWriter.maxDelayMillis
	      ...
	      event1 must startWith regex
   			s""""\\$$STATE_VISIT","\\d*","${sid}","\\$$STATUS=Committed;\\$$STATE=state1","test1","(A|B)","((true)|(false))""""
	      
	      val event2 = lines.next()
	      event2 must startWith regex
   			s""""\\$$STATE_VISIT","\\d*","${sid}","key1=val1;key2=val2;\\$$STATE=state2;\\$$STATUS=Failed","test1","(A|B)","((true)|(false))""""
